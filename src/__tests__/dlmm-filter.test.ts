...
describe("isDlmmBuyWall", () => {
  it("accepts classic buy wall: 100% SOL, 75 SOL, range below current", () => {
  ...
  it("'either' direction accepts both above and below", () => {
  ...
  it("rejects when solValue is 0 (no SOL)", () => {
    const r = isDlmmBuyWall(
      snap({ solValue: 0, solFraction: 0 }),
      cfg
    );
    expect(r.matched).toBe(false);
  });
